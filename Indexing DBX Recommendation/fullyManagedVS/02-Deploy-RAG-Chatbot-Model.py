...
# MAGIC To run this demo, just select the cluster `dbdemos-llm-rag-chatbot-yulan_yan` from the dropdown menu ([open cluster configuration](https://e2-demo-field-eng.cloud.databricks.com/#setting/clusters/0219-024121-twvsfyli/configuration)). <br />
...
# index_name=f"{catalog}.{db}.databricks_pdf_documentation_vs_index"
...
## instead of SP, here we use personal token for the demo
host = "https://" + spark.conf.get("spark.databricks.workspaceUrl")
os.environ['DATABRICKS_TOKEN'] = dbutils.secrets.get("llm_scope", "e2_demo_token")
...
# MAGIC - Leverage databricks-mpt-30b-instruct foundation model endpoint
