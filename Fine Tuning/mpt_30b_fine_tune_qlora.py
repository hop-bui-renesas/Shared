...
# MAGIC %md
# MAGIC # Fine tune `mpt_30b` model from marketplace with QLORA
# MAGIC
# MAGIC Databricks hosts the `mpt_30b` in [Databricks marketplace](https://marketplace.databricks.com/). This is a tutorial to show how to fine tune the models on the [mosaicml/dolly_hhrlhf](https://huggingface.co/datasets/mosaicml/dolly_hhrlhf) dataset.
# MAGIC
# MAGIC Environment for this notebook:
# MAGIC - Runtime: 14.1 GPU ML Runtime
# MAGIC - Instance: `g5.8xlarge` on AWS, `Standard_NV36ads_A10_v5` on Azure, `g2-standard-8` or `a2-ultragpu-1g` on GCP
# MAGIC
...
catalog_name = "databricks_mpt_models" # Default catalog name when installing the model from Databricks Marketplace
version = 1

model_mlflow_path = f"models:/{catalog_name}.models.mpt_30b/{version}"

model_local_path = "/local_disk0/mpt_30b/"
model_output_local_path = "/local_disk0/mpt_30b_lora_fine_tune"
...
# MAGIC
# MAGIC In this section we will load the [mpt_30b](https://huggingface.co/mosaicml/mpt-30b) model installed from [Databricks marketplace](https://marketplace.databricks.com/) saved in Unity Catalog to local disk, quantize it in 4bit and attach LoRA adapters on it.
...
catalog_name = "llm_poc_catalog_westus2"

# COMMAND ----------

registered_name = f"{catalog_name}.models.mpt_30b_lora_fine_tune" # Note that the UC model name follows the pattern <catalog_name>.<schema_name>.<model_name>, corresponding to the catalog, schema, and registered model name
